...
| Capacidade | Status | API real | Evidência |
|---|---|---|---|
| Unir superfície ao sólido do bloco | ✅ **validado pelo botão em 2026-09-21** | `Model.Attach(1, Body[] by-ref, bAdd=true, fpcSide=2)` → void. O array é o **`Body` da superfície** (`surf.Faces[1].Item(1).Body` — os features de superfície não têm `.Body`), não o feature: com `StitchSurface[]` deu E_FAIL nos 2 lados, com o `Body[]` da mesma superfície anexou de 1ª. Anexar a superfície que o usuário preparou (a última `StitchSurface`), NUNCA uma `CopySurface` das faces dela (a cópia deita em cima da original → E_FAIL). Rim exatamente na face do bloco basta | EE03 (20→37 faces), Teste.par (14→27) |
| Estender superfície | 🟡 API achada, chamada não funciona | `ExtendSurfaces` mora em **`Constructions.Item(i)`** (ConstructionModel), não em `Constructions` nem no Model (DISP_E_UNKNOWNNAME). `AddEx(n, Edge[], dist, tipo)` em SÍNCRONO: E_FAIL p/ tudo testado; `Add1` com face-alvo: E_INVALIDARG. Estender continua manual | sondagem 2026-09-21 |
| Arestas abertas de uma superfície | ✅ | `Edge` **não tem `Faces`** — `Edge.GetFaces([out] n, [out] Face[])` com placeholder `new Face[0]` by-ref; n=1 aberta, 2 costurada | sondagem 2026-09-21 |
| Unir via booleana **REGISTRADA na árvore** — ❌ **não serve p/ superfície** | ❌ E_NOINTERFACE em TODA rodada com superfície como ferramenta (é booleana entre corpos) | `Model.BooleanFeatures.Add(NumberOfTools:int, Tools:VARIANT tipado, Function:int=3/*seBooleanUnite*/, [opt]PlaneSide) → BooleanFeature`; coleção via `InvokeMember(GetProperty)` (acesso dinâmico dá E_NOINTERFACE) | dump:8736/8743 (`_IBooleanFeaturesAuto.Add`) |
...
| **Seleção de FACE de uma OCORRÊNCIA, direto na montagem, sem entrar em contexto** | ✅ (2026-07-21, log `101106`) | `AssemblyDocument.SelectSet.Item(i)` devolve um objeto EMBRULHO, não a `Face` crua — tem `.Object` (a `Face` de verdade: Area/Body/Edges/GetRange/Vertices) e `.ImmediateParent` (a `Occurrence` dona: Name/OccurrenceDocument/PartFileName/GetTransform), além de `.GetMatrix`/`.GetOccurrencesInPath`/`.Style`/`.Type`. Desembrulhar via `.Object` antes de chamar `GetRange`/`TryGetRangeMm` — sem isso toda leitura de geometria falha (`DISP_E_UNKNOWNNAME`) mesmo com `SelectSet.Count` correto. `.ImmediateParent` é um atalho direto pra ocorrência dona (mais confiável que casar por nome de documento). | log `101106`, SPY ao vivo |
| **Pintar face — o que falhava no botão (2026-09-21)** | ✅ validado pelo botão | A seleção traz quase sempre 1 item que não é Face; pegar o corpo de `faces[0]` cru derrubava tudo. Filtrar só faces (desembrulhar `.Object`), agrupar por corpo (1 `SetFacesStyle` por corpo — corpos misturados = E_FAIL), conferir `Face.Style.StyleName` de volta. `FaceStyle` não tem `Name`, é `StyleName`; `FaceStyles.Item(nome)` inexistente = 0x80040B50. A cor sobrevive ao `FaceOffsets.AddEx` | peça de teste + botão |
| **Pintar face SEM override próprio (a maioria)** | ✅ causa raiz achada (2026-07-21, log `101106`) | Escrever direto em `Face.Style.Diffuse{Red,Green,Blue}` só funciona se `Face.Style` já não for `null` — na maioria das faces de queima é `null` (sem override PRÓPRIO), e escrever num `Style` nulo falha SILENCIOSAMENTE (sem exceção visível pro chamador). O comando real que CRIA o override: `Body.SetFacesStyle(NumberOfFaces:int, FacesArray:SAFEARRAY(IDispatch), Style:FaceStyle) → HRESULT` (interface `_IDMDBody`); o `FaceStyle` vem de `Document.FaceStyles.Add(Name:BSTR, Parent:BSTR) → FaceStyle` (ou `.Item(Name)` se já existir) — `FaceStyle.DiffuseRed/Green/Blue` são `float` get/put. | dump:30800 (`SetFacesStyle`), 30022 (`FaceStyles.Add`), 28795-28800 (`FaceStyle.Diffuse*`) |
